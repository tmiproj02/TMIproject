...
public class CashSptListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Cash> spentList = new ArrayList<Cash>();
		...
		// 페이징 처리에 필요한 변수들 ----------
				...
				// 페이징 처리
				int listCount = crs.getsptListCount(m); //게시글 수 (물론 selectList로 해도 되지만, ListCount는 개수만 가져온다.)
		...
		try {
			spentList = crs.selectSptList(currentPage, limit, m);
		
			page = "views/personBUY/billingHistorySpt.jsp";
			
			request.setAttribute("spentList", spentList);
