				...
				<div style="margin-top:20px">
					<div class="padding-15">
						<ul class="sell-ing">
							...
							<li>
								<a href="/semi/dSelect.do?state=s2">완료 &nbsp; <span class="selling-history select"><%=s2 %></span></a>
							</li>
							<li>
					...
					<div class="padding-15" style="margin-top:10px">
						<div class="detail-box">
							...
							<div class="detail-list" style="padding:0">
								<%for(DealMng d : list){%>
								<%if(d.getProgress().equals("완료")){ count++;%>
								<%
									...
									<div style="width:10%; border-right:none;"><div style="margin:30px 0">
										<select onchange="if(this.value) location.href=(this.value);" name="" id="" style="border-radius:5px;font-size:13px;">
											<%if(d.getProgress().equals("진행중")){ %>
												<option value="/semi/uProgress.do?progress=진행중&dmcode=<%=d.getDmcode()%>&state=s2">진행중</option>
												<option value="/semi/uProgress.do?progress=완료&dmcode=<%=d.getDmcode()%>&state=s2">완료</option>
												<option value="/semi/uProgress.do?progress=취소&dmcode=<%=d.getDmcode()%>&state=s2">취소</option>
						...
						<% if(list != null){ int count = -1;%>
						<% for(DealMng d : list){if(d.getProgress().equals("완료")){ count++;}}%>
						<% if(count>-1){ %>
							<div class="paging">
